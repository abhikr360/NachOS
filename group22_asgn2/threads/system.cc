...
TimerInterruptHandler(int dummy)
{
    ...
    if (interrupt->getStatus() != IdleMode) {
        ...
        if(scheduler->schedulingAlgo >= 3 && scheduler->schedulingAlgo<=6){
            // printf("***********************************************\n");
            if((stats->totalTicks - currentThread->startCpuBurst) >= stats->TimerTicks){
                currentThread->hasQuantaExpired=1;
                interrupt->YieldOnReturn();
            }
        }
        //printf("[%d] Timer interrupt.\n", stats->totalTicks);
        else if(!(scheduler->schedulingAlgo == 1 || scheduler->schedulingAlgo == 2) )
             interrupt->YieldOnReturn();
