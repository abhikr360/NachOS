...
class NachOSThread {
  private:
	...
	int childPidArray[MAX_CHILDREN];			//contains pids of children
	bool childHasExited[MAX_CHILDREN];		//childhasexited[i] = true if child with pid  = 																			childpidarray[i] has already exited
	int childExitStatus[MAX_CHILDREN];		//contains exitstatus of already exitted children
	int childIndex;											//total number of childrens yet made
  public:
    NachOSThread * parent;
	void addChild(int PID);								// add a new child

	void exitChild(int PID, int exitCode);			//call when child has to exit

	int isChild(int PID);									//returns true if PID is child of this thread

	int joinWithChild(int PID, int index);
