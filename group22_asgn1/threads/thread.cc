...
globalThread[pid]=this;
#endif
}

void
NachOSThread::addChild(int PID)
{
	childPidArray[childIndex] = PID;
	childHasExited[childIndex] = false;
    //printf("childIndex %d and child pid %d\n", childIndex, childPidArray[childIndex]);
...
void 
NachOSThread::exitChild(int PID, int exitCode)
{	//printf("level1\n");
	...
	{	//printf("level2\n");
		...
		{	
				//printf("level3\n");
				childExitStatus[i] = exitCode;
				childHasExited[i] = true;
		}
	}
}
...
{
    //printf("inside isChild\n");
	for(int i = 0; i<childIndex; i++)
	{
        //printf("childPidArray[i] : %d and pid : %d\n", childPidArray[i], PID);

		if(childPidArray[i] == PID)
		{	
           // printf("childPidArray[i] : %d and pid : \n", childPidArray[i], PID);
				return i;
		}
	}
	return -1;
...
int
NachOSThread::joinWithChild(int PID, int index)
{
    //printf("inside joooooin wiiiiith chiiiild XDXDXDXDXDXD \n");
	if(!childHasExited[index])
	{
...
//----------------------------------------------------------------------
//void
//NachOSThread::setRegisterVal(int regno,int val){
//userRegisters[regno]=val;
//}
void
NachOSThread::setReturnToZero(){
//	setRegisterVal(2,0);
	userRegisters[2]=0;
}
NachOSThread::~NachOSThread()
...
NachOSThread::SaveUserState()
{
    if (stateRestored) {
	  ...
	  this->userRegisters[i] = machine->ReadRegister(i);
       stateRestored = false;
    }
}
...
void
NachOSThread::GoToSleep(long waketime){
	//ListElement * temp, *temp1, *temp2;
	WaitingList->SortedInsert(currentThread, waketime);
...
void
NachOSThread::DeleteUtil(){
	scheduler->DeleteThreadToBeDestroyed();
}
#endif
